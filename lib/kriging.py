...
class kriging():
    def __init__(self, x, y):
        ...
        self.xs, self.ys, self.min_x, self.max_x, self.min_y, self.max_y. self.y_log = self.scale_trainingdata(x, y)
    ...
    def checkinputsdimension__(self, x, y):
        ...
        assert (x.shape[0] != y.shape[0]), "The number of data in x must be the same with y"
        return x, y
